...
namespace CmplxConExt
{
	...
	void write(const std::string& content) {
		std::string call = "write(\"" + content + "\")";
		emscripten_run_script(call.c_str());
	};
	void write(const std::wstring& content) {
		std::string call = "write(\"" + toMbsString(content) + "\")";
		emscripten_run_script(call.c_str());
	};
	...
	void writeLine(const std::string& content) {
		std::string call = "writeLine(\"" + content + "\")";
		emscripten_run_script(call.c_str());
	};
	void writeLine(const std::wstring& content) {
		std::string call = "writeLine(\"" + toMbsString(content) + "\")";
		emscripten_run_script(call.c_str());
	};
	...
	std::wstring resolveReaded() {
		return toWcsString(emscripten_run_script_string("resolveReaded()"));
	};
}
